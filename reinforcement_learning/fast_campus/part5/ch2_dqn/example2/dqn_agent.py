...
class DQNAgent(nn.Module):
    ...
    def train(self):
        ...
        actions_array = np.stack(actions, axis=0)   # (n_batch)
        rewards_array = np.stack(rewards, axis=0, dtype=np.int64)   # (n_batch)      # (n_batch)
        next_states_array = np.stack(next_states, )     # (n_batch, d_state)
        ...
        rewards_tensor = torch.from_numpy(rewards_array)    # (n_batch)
...
if __name__ == "__main__":
    ...
    for _ in range(init_replay_buffer_size):
        a = np.random.choid(env.acion_space.n)
    ...
    for step_train in range(config.train_env_steps):
        ...
        if step_count % config.target_update_period == 0:
