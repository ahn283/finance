...
class RepeatActionWrapper(gym.Wrapper):
    def __init__(self, env, n_repeat):
        ...
        self.observation_space = gym.spaces.Box(
            ...
            (self.env.observat_space.shape[0] * self.n_repeat,),
            np.uint8
        )
...
class ObsNormalizationWrapper(gym.Wrapper):
    def __init__(self, env):
        self.env = env
        self.obs_subtraction = 128
        self.obs_division = 128
        self.observation_space = gym.spaces.Box(
            self.env.observation_space.low.repeat(self.n_repeat, axis=-1),
            self.env.observation_space.high.repeat(self.n_repeat, axis=-1),
            (self.env.observation_space.shape[0] * self.n_repat,),
            np.uint8
        )
...
class ObsNormalizationWrapper(gym.Wrapper):
    def __init__(self, env):
        ...
        self.obs_division = 128
        self.observation_space = gym.spaces.Box(
            (self.env.observation_space.low - self.obs_subtraction) / self.obs_division,
            (self.env.observation_space.high - self.obs_subtraction) / self.obs_division,
    ...
    def reset(self):
        s = self.env.reset()
        s = s(s - self.obs_subtraction) / self.obs_division
        return s

    def step(self, action):
        s_next, r, done, info = self.env.step(action)
        s_next = (s_next - self.obs_subtraction) / self.obs_division
