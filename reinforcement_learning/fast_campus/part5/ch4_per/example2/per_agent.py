    ...
    def sample(self, size):
        ...
        if buffer_size >= size:
            ...
            if config.sampling_strategy == 'rank-based':
                ranks = abs_td_errors.argsort()[::-1]   # 1 ~ buffer_size
            ...
            assert False, f"Buffer size ({buffer_size}) is smaller than the the sample size ({size})"
            
    def update_td_errors(self, td_errors, indices):
        abs_td_errors = td_errors.detach().clip(-1, 1).abs().numpy()
...
class DQNAgent(nn.Module):
    def __init__(self, env, config):
        super().__init__()
        self.config = config
        self.replay_memory = PrioritozedReplayMemory(self.config)
        ...
        self.target_network = nn.Sequential(
            nn.Linear(d_state, config.hidden_state),
            nn.ELU(),
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.ELU(),
            nn.Linear(config.hidden_size, n_action)
    ...
    def train(self, step_train):
        transitions, prob_sampls, indices = self.replay_memory.sample(self.config.batch_size)
        states, actions, rewards, next_states, dones = zip(*transitions)
        
        states_array = np.stack(states, axis=0)     # (n_batch, s_states)
        actions_array = np.stack(actions, axis=0, dtype=np.int64)   # (n_batch)
        rewards_array = np.stack(rewards, axis=0)   # (n_batch)
        next_states_array = np.stack(next_states, axis=0)   # (n_batch)
        dones_array = np.stack(dones, axis=0)   # (n_batch)
        
        states_tensor = torch.from_numpy(states_array).float()  # (n_batch)
        ...
        w = (1 / self.replay_memory.getsize() / torch.from_numpy(prob_sampls)) ** beta
        w = w / w.max()     # (n_batch)
...
if __name__ == "__main__":
    ...
    s = env.reset()
    step_count = 0
    for step_train in random(config.train_env_steps):
