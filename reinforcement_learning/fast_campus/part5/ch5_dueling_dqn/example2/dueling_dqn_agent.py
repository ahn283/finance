...
class DuelingDQNAgent(nn.Module):
    ...
    def train(self):
        transitions = self.replay_memory(self.config.batch_size)
        ...
        next_Qs = self.target_network(next_states_tensor)   # (n_batch, n_action)
...
def eval_agent(config, env, agent):
    ...
    step_count_avg = score_sum / config.num_eval_episode
...
if __name__ == "__main__":
    ...
    logdir = f"logdir/{dt_now.strftime('%y-%m-%_%H-%M-%S')}"
