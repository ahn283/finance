...
class Agent(nn.Module):
    ...
    def add_to_batch(self, s, pi_old, a, r, s_next, done):
        ...
        if not done:
            ...
            # when trajectory is done before it is full, append the last data until the end
    ...
    def action(self, x):
        ...
        with torch.no_grad():
            x = torch.from_numpy(x).float().reshape(1, -1)  # turing it into 2-dimensitonal matrix
    ...
    def train(self):
        for k in range(self.config.k_epoch):
            ...
            state_next_seq_array = np.array([trajectory['state_next'] for trajectory in minibatch])
            done_seq_array = np.array([trajectory['done'] for trajectory in minibatch])     # (n_batch, n_seq)
            
            state_seq_tensor = torch.from_numpy(state_seq_array).float().transpose(0, 1)
            pi_old_seq_tensor = torch.from_numpy(pi_old_seq_array).transpose(0, 1)
            ...
            # mask for updating policy, unitl the transition that its done is True
            ...
            pi_old_chosen = pi_old_seq_tensor.gather(dim=-1, index=action_seq_tensor.unsqueeze(-1))
            pi_old_chosen = pi_old_chosen.squeeze(-1)
            ...
            loss_exp = -torch.mean(
                update_mask * torch.sum(-pi * torch.log(pi + 1e-15), dim=-1)    # (n_seq, n_btch, n_action) -> (n_seq, n_batch)
            )
