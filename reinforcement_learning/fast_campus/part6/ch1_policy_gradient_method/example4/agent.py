...
class Agent(nn.Module):
    ...
    def train(self):
        ...
        valid_seq_lengths = update_mask.sum(dim=0).int().tolist()  # (n_batch)
        from_valid_seq_length_to_1 = torch.zeros(self.config.seq_length, self.config.batch_size)  # (n_seq, n_batch)
        for i_batch, valid_seq_length in enumerate(valid_seq_lengths):
            from_valid_seq_length_to_1[:valid_seq_length, i_batch] = (
                valid_seq_length - torch.arange(valid_seq_length)
            )

        gamma_power_seq = torch.pow(self.config.gamma, from_valid_seq_length_to_1)  # (n_seq, n_batch)
        n_step_td = (
            calc_return_seq_tensor(self.config.gamma, update_mask * reward_seq_tensor)
        ...
        #loss_critic = torch.mean(update_mask * criterion(value, value_target))
