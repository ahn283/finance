...
class Agent(nn.Module):
    ...
    def action(self, x):
        # used when sampling an action from a state
        with torch.no_grad():
    ...
    def train(self):
        state_seq_array = np.array([trajectory['state'] for trajectory in self.batch])
        action_seq_array = np.array([trajectory['action'] for trajectory in self.batch], dtype=np.int64)
        reward_seq_array = np.array([trajectory['reward'] for trajectory in self.batch])
        state_next_seq_array = np.array([trajectory['state_next'] for trajectory in self.batch])
        done_seq_array = np.array([trajectory['done'] for trajectory in self.batch])
        
        state_seq_tensor = torch.from_numpy(
            state_seq_array
        ).float().transpose(0, 1) 
        action_seq_tensor = torch.from_numpy(action_seq_array).transpose(0, 1)
        reward_seq_tensor = torch.from_numpy(reward_seq_array).float().transpose(0, 1)
        state_next_seq_tensor = torch.from_numpy(state_next_seq_array).float().transpose(0, 1)
        done_seq_tensor = torch.from_numpy(done_seq_array).float().transpose(0, 1)
        
        # mask for updating policy until the transition that its done is True
        update_mask = done_seq_tensor.roll(1, dims=0)
        update_mask[0, :] = 0
        update_mask = 1 - update_mask
        
        pi, value = self.forward(state_seq_tensor)
        _, value_next = self.forward(state_next_seq_tensor)
        value = value.squeeze(-1)
        value_next = value_next.squeeze(-1)
        
        valid_seq_length = update_mask.sum(dim=0).int().tolist()
        from_valid_seq_length_to_1 = torch.zeros(self.config.seq_length, self.config.batch_size)
        for i_batch, valid_seq_length in enumerate(valid_seq_length):
            from_valid_seq_length_to_1[:valid_seq_length, i_batch] = (
                valid_seq_length - torch.arange(valid_seq_length)
            )
            
        gamma_power_seq = torch.pow(self.config.gamma, from_valid_seq_length_to_1)
        n_step_td = (
            calc_return_seq_tensor(self.config.gamma, update_mask * reward_seq_tensor) + gamma_power_seq * value_next[-2:-1, :].detach()
        )
        
        pi_chosen = pi.gather(dim=-1, index=action_seq_tensor.unsqueeze(-1))
        pi_chosen = pi_chosen.squeeze(-1)
        
        value_target = (
            reward_seq_tensor + self.config.gamma * (1 - done_seq_tensor) * value_next.detach()
        )
        
        if self.config.use_n_step_advantage:
            advantage = n_step_td - value.detach()
        else:
            advantage = value_target - value.detach()
            
        criterion = nn.SmoothL1Loss(reduction='none')
        loss_critic = torch.mean(update_mask * (value_target - value) ** 2)
        loss_actor = -torch.mean(update_mask * advantage * torch.log(pi_chosen + 1e-15))
        loss_exp = -torch.mean(
            update_mask * torch.sum(-pi * torch.log(pi + 1e-15), dim=-1)
        )
