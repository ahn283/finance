...
class Env:
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            # 'y' should be increased by 1 or stay the same when it is at the most left column
            self.agent_pos['y'] = min(
                self.agent_pos['y'] + 1,
                self.y_max
            )
        elif action == 3:
            # 'x' should be decreased by 1 or stay the same when it is a the most left column
    ...
    def transition_probability(self, s, a, s_next):
        y, x = np.where(s == 1)     # get agent pos
        ...
        is_correction_transition = (
            y_next_temp == y_next and
            x_next_temp == x_next
        )
        
        if is_correction_transition:
            return 1.0
        else:
            return 0.0
