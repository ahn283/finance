...
class Env:
    ...
    def exploring_reset(self):
        while True:
            ...
            if not (
                ...
                (y_random in self.drift_area['y'] and x_random in self.drift_area['x'])
            ):
                break
        self.state = np.zeros([3, 4])
        self.state[self.goal_pos['y'], self.goal_pos['x']] = -1
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            # 'y' should be increased by 1 or stay the same when it is at the most right column
            self.agent_pos['y'] = min(
                self.agent_pos['y'] + 1,
                self.y_max
            )
        elif action == 3:
            # 'x' should be decreased by 1 or stay the same when it si at the most left column
