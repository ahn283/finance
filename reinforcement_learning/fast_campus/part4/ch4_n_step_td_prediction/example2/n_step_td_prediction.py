...
def n_step_td_value_prediction(env, policy, n, color):
    ...
    for loop_count in range(1000):
        ...
        # add new point
        plot_buffers['x'].append(loop_count)
        plot_buffers['y'].append(value_vector[0])
        if loop_count > 0:
            # draw a new line
            plt.plot(plot_buffers['x'], plot_buffers['y'], color=color)
            # remove drawed point
            plot_buffers['x'].pop(0)
            plot_buffers['y'].pop(0)
...
if __name__ == "__main__":
    ...
    value_vecotr_5 = n_step_td_value_prediction(env, policy, 5, color='blue')
