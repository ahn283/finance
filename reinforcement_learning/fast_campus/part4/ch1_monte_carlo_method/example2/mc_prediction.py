...
def mc_action_value_prediction(env, policy):
    ...
    retunrs = [[{'n': 0, 'avg': 0} for a in env.action_space] for s in env.state_space]
    ...
    for loop_count in range(10000):
        ...
        for t in range(step_count):
            ...
            n_prev, avg_prev = retunrs[i_s_t][i_a_t]['n'], retunrs[i_s_t][i_a_t]['avg']
            retunrs[i_s_t][i_a_t]['avg'] = (avg_prev * n_prev + g_t) / (n_prev + 1)
            retunrs[i_s_t][i_a_t]['n'] = n_prev + 1
            action_value_matrix[i_s_t][i_a_t] = retunrs[i_s_t][i_a_t]['avg']
