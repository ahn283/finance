...
def read_csv(file_name, sep=',', filter_data=True, fix_open_price=False):
    ...
    with open(file_name, 'rt', encoding='utf-8') as fd:
        ...
        for row in reader:
            ...
            # fix open price for current var to match close price for the previous bar
            ...
            v.append(pv)
            prev_vals = vals
    print("Read one, got %d rows, %d filtered. %d open price adjusted"
...
def load_year_data(year, basedir='data'):
    y = str(year)[-2:]
    result = {}
    for path in glob.glob(os.path.join(basedir, "*_s*.csv" %y)):
