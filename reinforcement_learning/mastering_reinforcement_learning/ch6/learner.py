...
class Learner:
    ...
    def optimize(self):
        # sampling from replay buffer and updating the counters we keep
        ...
        if samples:
            ...
            # unpack the samples and reshape them
            ...
            gammas = np.array([sample[5] for sample \
                        in samples]).reshape((N,))
            # we create the masks to only update the Q value
            # for the action selected in the experience tuple
            masks = np.zeros((N, n_actions))
            masks[np.arange(N), actions] = 1
            dummy_labels = np.zeros((N,))
            
            # we prepare the inputs to the trainable Q network and call the fit function
            # double DQN
