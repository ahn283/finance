...
def fillnull(df):
    ...
    ## Embarked
    df[['Embaked']] = df[['Embarked']].fillna(df['Embarked'].mode()[0], axis=0)
...
def prep(df):
    ...
    name_sub.replace([' Master', ' Major', ' Col', ' Capt', ' Sir', ' Mille', ' Jonkheer'], 'Officer', inplace=True)
...
def evaluation_class_PrecisionRecall(Y_true, P_pred, figsize=(10, 5)):
    pre, rec, threshold = precision_recall_curve(Y_true, P_pred)

    plt.figure(figsize=(figsize))
    plt.plot(threshold, pre[:-1], label='Precision')
