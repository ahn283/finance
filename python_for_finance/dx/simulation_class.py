...
class simulation_class(object):
    ''' Provoding base methods for simulation classes.
    ...
    mar_env : instance of market_environment
        markket environment data for simulation
    corr : bool
    ...
    def __init__(self, name, mar_env, corr):
        ...
        try:
            ...
            self.time_grid = mar_env.get_lst('time grid')
        except:
            self.time_grid = None
        try:
        ...
        if corr is True:
            ...
            self.cholesky_matrix = mar_env.list('cholesky_matrix')
            self.rn_set = mar_env.get_list('rn_set')
            self.random_numbers = mar_env.get_list('random numbers')
    ...
    def generate_time_grid(self):
        ...
        # enhance time_grid by start, end, and special dates.
        if start not in time_grid:
            # insert start date if not in list
            time_grid.insert(0, start)

        if end not in time_grid:
            # insert end if not in list
