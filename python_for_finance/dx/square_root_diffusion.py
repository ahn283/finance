...
class square_root_diffusion(simulation_class):
    ...
    the Cox-Ingersoll-Ross (1985) sqaure-root diffusion model
    ...
    def generate_paths(self, fixed_seed=True, day_count=365.):
        ...
        for t in range(1, len(self.time_grid)):
            ...
            paths_[t] = (paths_[t - 1] + self.kappa * (self.theta - np.maximum(0, paths_[t - 1, :])) * dt + np.sqrt(np.maximum(0, paths_[t - 1, :])) * self.volatility * np.sqrt(dt) * ran)
