...
def clean_up_text(t, numbers=False, punctuation=False):
    ''' Clean up a text, e.g. HTML document
    from HTML tags and also cleans up the text body
    '''
...
def tokenizer(text, min_char=3, lemma=True, stop=True, numbers=False):
    ''' Tokenizer a text and implements some transformation'''
...
def generate_word_cloud(text, no, name=None, show=True):
    ''' Genertes a word cloud bitmap given a text document (string).
    It uses the Term Frequency (TF)
    Inverse document Frequency (IDF) vectorization approach to derive the improtance of a word representation
    by the sie of the word in the word cloud
    ...
    vec = TfidfVectorizer(min_df=2, analyzer='word', ngram_range=(1, 2), stop_words='english')
    ...
    wordcloud = WordCloud(max_font_size=110, background_color='white', width=1024, height=768, margin=10, max_words=150).generate(words)
    if show:
...
def generate_key_words(text, no):
    try:
        ...
        words = [ a for a in words if not a.isnumric()][:no]
