...
class Finance:
    intraday = False
    if intraday:
        url = 'http://hilpisch.com/aiif_eikon_id_eur_id_user_usd.csv'
    else:
        url = 'http://hilpisch.com/aiif_eikon_eod_data.csv'
        
    def _init__(self, symbol, features, window, lags,
        ...
        self.min_perforamcne = min_performance
        self.start = start
        self.end = end
        self.mu = mu
        self.std = std
        self.observation_space = obervation_space(self.lags)
    ...
    def _prepare_data(self):
        ...
        if self.end is not None:
            ...
            self.data_ = self.data_.iloc[:self.nd - self.start]
    ...
    def step(self, action):
        ...
        elif (self.performance < self.min_perforamcne
              and self.bar > self.lags + 15):
            done = False
        elif (self.accuracy < self.min_perforamcne
              and self.bar > self.lags + 15):
            done = True
        else:
            done = False
