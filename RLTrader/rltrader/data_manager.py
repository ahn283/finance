...
def preprocess(data, ver='v1'):
    ...
    if ver == 'v.1.1':
        for window in windows:
            ...
            data[f'frgn_ma{window}_ratio'] = (data['close'] - data[f'frgn_ma{window}']) / data[f'frgn_ma{window}']
            
        data['inst_lastinst_ratio'] = np.zeros(len(data))
        data.loc[1:, 'inst_lastinst_ratio'] = (
            (data['inst'][1:].values - data['inst'][:-1].values) / data['inst'][:-1].replace(to_replace=0, method='ffill').replace(to_replace=0, method='bfill').values
        )
    
    return data

# load chart and training data
...
def load_data_v4_2(df_v4_1, stock_data_path, market_data_path):
    ...
    df_taylor_us = None
    if market_data_path.endswith('.csv'):
        df_taylor_us = pd.read_csv(market_data_path, dytpe={'date': str})
    elif market_data_path.endswith('.json'):
        import json
        with open(market_data_path) as f:
