...
# 텐서플로우 버전 2를 사용 -> tensorflow 패키지 내 keras 패키지 사용
if os.environ.get('KEARAS_BACKEND', 'tensorflow') == 'tensorflow':
...
# plaidML 을 사용 -> keras 패키지 바로 사용
elif os.environ['KERAS_BACKEND'] == 'plaidml.keras.backend':
    from keras.models import Model
    from keras.layers import Input, Dense, LSTM, Conv1D, BatchNormalization, Dropout, MaxPool1D, Flatten
    from keras.optimizers import SGD

class Network:
    lock = threading.Lock()
    
    def __init__(self, input_dim=0, output_dim=0, lr=0.001, share_network=None, activation='sigmoid', loss='mse'):
    ...
    @classmethod
    def get_share_network(cls, net='dnn', num_steps=1, input_dim=0, output_dim=0):
...
class LSTMNetwork(Network):
    ...
    def get_network_head(inp):
        # cuDNN 사용을 위한 조건
        # http://www.tensorflow.org/api_docs/python/tf/keras/layres/LSTM
...
class CNN(Network):
    def __init__(self, *args, num_steps=1, **kwargs):
        ...
        if self.shared_network is None:
            ...
            output = self.get_newwork_head(inp).output
        else:
            inp = self.shared_network.input
            output = self.shared_network.output
        output = Dense(
